...
while ($r = $rows->fetch_assoc()) {
    ...
    if ($target_status === 'approved') {
        $stmt = $connect->prepare("UPDATE transaksi SET status = 'approved' WHERE id_anggota = ? AND keterangan LIKE ?");
        if ($stmt) {
        ...
        $stmt = $connect->prepare("UPDATE transaksi SET status = 'rejected', keterangan = CONCAT(keterangan, ' | Ditolak: ', ?) WHERE id_anggota = ? AND keterangan LIKE ?");
        if ($stmt) {
