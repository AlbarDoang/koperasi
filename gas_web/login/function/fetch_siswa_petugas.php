...
try {
	...
	$sql = "SELECT * FROM siswa WHERE NOT id_jurusan='1'";
	$sql = "SELECT * FROM pengguna WHERE NOT id_jurusan='1'";
	$params = [];
	if (!empty($_POST['search']['value'])) {
		$sql .= " AND (id_tabungan LIKE :q OR nama LIKE :q)";
		$params[':q'] = '%' . $_POST['search']['value'] . '%';
	}

	$cols = [0 => 'id', 1 => 'id_tabungan', 2 => 'no_pengenal', 3 => 'nama', 4 => 'jk', 5 => 'no_wa', 6 => 'kelas'];
	$orderCol = 'id';
	$orderDir = 'DESC';
	...
	$countSql = "SELECT COUNT(*) FROM siswa WHERE NOT id_jurusan='1'" . (!empty($_POST['search']['value']) ? " AND (id_tabungan LIKE :q OR nama LIKE :q)" : '');
	$countSql = "SELECT COUNT(*) FROM pengguna WHERE NOT id_jurusan='1'" . (!empty($_POST['search']['value']) ? " AND (id_tabungan LIKE :q OR nama LIKE :q)" : '');
	...
	$extra = ob_get_clean();
	if (!empty($extra)) @file_put_contents(__DIR__ . '/../logs/fetch_siswa_petugas_output_debug.log', date('c') . " stray_output:\n" . $extra . "\n", FILE_APPEND);
	echo json_encode($out, JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
	$extra = ob_get_clean();
	@file_put_contents(__DIR__ . '/../logs/fetch_siswa_petugas_error.log', date('c') . " ajax error: " . $e->getMessage() . "\nextra:\n" . $extra . "\n", FILE_APPEND);
	$draw = isset($_POST['draw']) ? intval($_POST['draw']) : 1;
